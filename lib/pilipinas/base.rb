   ...
   class Base
      ...
      class << self
         ...
         def assoc_collection(options)
            ...
            load_file(File.join(File.dirname(__FILE__), '..', 'data', "#{dir}", "#{code}.yml"))
         end
         ...
         def load_file(file)
            ...
            yaml.each { |hash, opts| add(new(hash)) } if yaml
            data
         end
         ...
         def parse_attributes(attr, val)
            raise Pilipinas::UnknownAttribute, "Invalid attribute '#{attr}'." unless [:code, :name].include?(attr.to_sym)
